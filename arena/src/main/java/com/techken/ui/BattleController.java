...
public class BattleController {
    ...
    @FXML
    private AnchorPane battleArenaRoot;
    @FXML
    ...
    public void initMatch(BaseCharacter playerCharacter, BaseCharacter cpuCharacter) {
        ...
        try {
            // 1. Get the names and remove spaces
            // "Devil Jin" -> "DevilJin", "Manny Pacquiao" -> "MannyPacquiao"
            String pName = playerCharacter.getName().replace(" ", "");
            String cName = cpuCharacter.getName().replace(" ", "");

            // 2. Map Player file details based on image_775d9a.png
            String pExt = pName.equalsIgnoreCase("DevilJin") ? ".jpg" : ".png";

            // Fix for Heihachi: if name is "HeihachiMishima", trim to "Heihachi" to match file
            if (pName.startsWith("Heihachi")) pName = "Heihachi";

            // 3. Map CPU file details
            String cExt = cName.equalsIgnoreCase("DevilJin") ? ".jpg" : ".png";
            if (cName.startsWith("Heihachi")) cName = "Heihachi";

            // 4. Construct Paths (Using capital '-Icon' as seen in image_775d9a.png)
            String pPath = "/fightericons/" + pName + "-Icon" + pExt;
            String cPath = "/fightericons/" + cName + "-Icon" + cExt;

            // 5. Load the Images
            InputStream pStream = getClass().getResourceAsStream(pPath);
            InputStream cStream = getClass().getResourceAsStream(cPath);

            if (pStream != null) {
                playerIdle = new Image(pStream);
                playerImageView.setImage(playerIdle);
                playerHurt = playerIdle;
            } else {
                System.out.println("Missing Player Image: " + pPath);
            }

            if (cStream != null) {
                cpuIdle = new Image(cStream);
                cpuImageView.setImage(cpuIdle);
                cpuHurt = cpuIdle;
                cpuImageView.setScaleX(-1); // Flip CPU
            } else {
                System.out.println("Missing CPU Image: " + cPath);
            }
        ...
        private void executeAction (BaseCharacter attacker, BaseCharacter defender, BaseSkill skill, String tag, State
        nextState){
        ...
        private String applySkill (BaseSkill skill, BaseCharacter attacker, BaseCharacter defender, String tag){
            ...
            if (damage > 0) {// add ta ug if because 'GuardAction' skills deal nothin
                if (defender == playerCharacter) {
                    triggerHurtAnimation(playerImageView, playerHurt, playerIdle);
                } else {
                    triggerHurtAnimation(cpuImageView, cpuHurt, cpuIdle);
                }
